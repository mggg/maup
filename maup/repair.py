...
def autorepair(
    geometries0_df, fill_gaps=True, min_rook_length=None, holes_to_keep=None
):
    ...
    if holes_to_keep is not None:
        if holes_to_keep.crs != geometries_df.crs:
            raise Exception(
                "holes_to_keep must be in the same CRS as the shapefile being repaired."
            )
    ...
    if len(disconnected_df) > 0:
        ...
        if (
            len(disconnected_poly_indices) > 0
            ...
            for g_ind in disconnected_poly_indices:
                ...
                component_num_list = [
                    x for x in range(len(reconstructed_df["geometry"][g_ind].geoms))
                ]
                component_areas = []
                ...
                for i in range(
                    excess
                ):  # Check that the ith smallest component has small enough area find a better polygon to add it to.
                    c_ind = component_areas_sorted[i][0]
                    this_fragment = reconstructed_df["geometry"][g_ind].geoms[c_ind]
                    if (
                        component_areas_sorted[i][1] < 0.0001 * big_area
                    ):  # Less than 0.01%
                ...
                if len(component_num_list) == 1:
                    reconstructed_df["geometry"][g_ind] = reconstructed_df["geometry"][
                        g_ind
                    ...
                    print(
                        ...
                        g_ind,
                        " was badly disconnected and redistributed to other polygons!",
                    )
    ...
    if holes_to_keep is not None:
        ...
        reconstructed_spatial_index = STRtree(
            reconstructed_df["geometry"]
        )  # Build STRtree for all the reconstructed geometries
        reconstructed_index_by_iloc = dict(
            (i, list(reconstructed_df.index)[i]) for i in range(len(reconstructed_df))
        )  # Build indexing dictionary

        for keeper in holes_to_keep:
            possible_geom_integer_indices = [
                *set(numpy.ndarray.flatten(reconstructed_spatial_index.query(keeper)))
            ]
            possible_geom_indices = [
                reconstructed_index_by_iloc[k] for k in possible_geom_integer_indices
            ]
...
def building_blocks(geometries0_df):
    ...
    pieces_df = geopandas.GeoDataFrame(
    ...
    g_spatial_index = STRtree(
        geometries_df["geometry"]
    )  # Build STRtree for the main geometries
    ...
    for i in progress(pieces_df.index, len(pieces_df.index)):
        ...
        for j in possible_geom_indices:
            if (
                ...
                .distance(geometries_df["geometry"][j])
                == 0
            ):
                pieces_df["polygon indices"][i] = pieces_df["polygon indices"][i].union(
                    {j}
                )
...
def reconstruct_from_overlap_tower(geometries0_df, overlap_tower0):
    ...
    for ind in overlap_tower[0].index:
        ...
        geometries_df["geometry"][this_poly_ind] = unary_union(
    ...
    for ind in geometries_df.index:
        geometries_df["num components orig"][ind] = num_components(
            geometries0_df["geometry"][ind]
        )
        geometries_df["num components refined"][ind] = num_components(
    ...
    for i in range(1, max_overlap_level):
        ...
        o_spatial_index = STRtree(
            overlaps_df["geometry"]
        )  # Build STRtree for the overlaps
        ...
        for g_ind in geometries_disconnected_df.index:
            ...
            geom_finished = (
                False  # Only keep adding things until it gets connected again
            )
            ...
            geometries_df["geometry"][g_ind] = geometries_disconnected_df["geometry"][
                g_ind
            ]
        ...
        for o_ind in progress(
            overlaps_df_unused_indices, len(overlaps_df_unused_indices)
        ):
            ...
            if len(shared_perimeters) > 0:
                ...
                print(
                    "Couldn't find a polygon to glue a component of intersection ",
                    o_ind,
                    " to",
                )
...
def close_gaps(geometries0_df, holes0_df):
    ...
    while fill_complete == False:
        ...
        hole_boundaries_df = intersections(
            holes_df, geometries_df, output_type="geodataframe"
        )
        ...
        for ind in hole_boundaries_df.index:
            if hole_boundaries_df["geometry"][ind].geom_type == "MultiLineString":
                hole_boundaries_df["geometry"][ind] = linemerge(
                    hole_boundaries_df["geometry"][ind]
                )
        ...
        for h_ind in progress(holes_df.index, len(holes_df.index)):
            ...
            this_hole_boundaries_df = hole_boundaries_df[
                hole_boundaries_df["source"] == h_ind
            ]
            ...
            if len(set(this_hole_boundaries_df["target"])) <= 3:  # Fill the hole!
                ...
                (
                    ...
                    piece_to_connect,
                    new_holes_this_time,
                ) = partial_fill_data(this_hole, this_hole_boundaries_df)
        ...
        if activity_this_round == True:  # Add any new holes for the next round!
            for k in range(len(new_holes)):
                ...
                hole_to_append_df = geopandas.GeoDataFrame(
                    columns=["polygon indices"],
                ...
                holes_df = pandas.concat(
                    [holes_df, hole_to_append_df], ignore_index=True
                )
...
def partial_fill_data(hole0, hole_boundaries_df0):
    ...
    for i in hole_boundaries_df.index:
        for j in hole_boundaries_df.index:
            if j > i:
                this_distance = hole_boundaries_df["geometry"][i].distance(
                    hole_boundaries_df["geometry"][j]
                )
    ...
    while poly_pair_found == False and len(distance_data_sorted) > 0:
        ...
        for i in range(2):
            boundary_point_coords.append(
                [x for x in hole_boundaries_df["geometry"][polys_to_connect[i]].coords]
            )
            vertices[i].append(
                Point(boundary_point_coords[i][0][0], boundary_point_coords[i][0][1])
            )
            vertices[i].append(
                Point(boundary_point_coords[i][-1][0], boundary_point_coords[i][-1][1])
            )

        for i in range(2):
            for j in range(2):
                vertices_nearest_point_pairs[i].append(
                    shapely.ops.nearest_points(
                        vertices[i][j], MultiPoint(boundary_point_coords[1 - i])
                    )
                )
                vertices_distances[i].append(
                    vertices_nearest_point_pairs[i][j][0].distance(
                        vertices_nearest_point_pairs[i][j][1]
                    )
                )
        ...
        sorted_distances = sorted(
            [
                (0, 0, vertices_distances[0][0]),
                (0, 1, vertices_distances[0][1]),
                (1, 0, vertices_distances[1][0]),
                (1, 1, vertices_distances[1][1]),
            ],
        ...
        for k in range(4):
            ...
            sorted_line_segments_0.append(
                LineString(list(vertices_nearest_point_pairs[i][j]))
            )
        ...
        for k in range(4):
            if hole.contains(sorted_line_segments_0[k]) or hole.boundary.contains(
                sorted_line_segments_0[k]
            ):
        ...
        if len(sorted_line_segments) > 1:
            for j in range(1, len(sorted_line_segments)):
                for i in range(j):
                    if (
                        sorted_line_segments[i].disjoint(sorted_line_segments[j])
                        and line_pair_found == False
                    ):
                        line_pair = [i, j]
        ...
        if line_pair_found == True:
            ...
            for k in range(len(first_split.geoms)):
                if first_split.geoms[k].intersects(sorted_line_segments[line_pair[1]]):
                    second_split = shapely.ops.split(
                        first_split.geoms[k], sorted_line_segments[line_pair[1]]
                    )
                else:
        ...
        if poly_pair_found == True:
            if (
                piece_to_connect.intersection(
                    hole_boundaries_df["geometry"][polys_to_connect[0]]
                ).length
                > piece_to_connect.intersection(
                    hole_boundaries_df["geometry"][polys_to_connect[1]]
                ).length
            ):
    ...
    if poly_pair_found == False:  # Not much to do at this point except join by shared
        ...
        for b_ind in hole_boundaries_df.index:
            ...
            shared_perimeters.append(
                (b_ind, hole_boundaries_df["geometry"][b_ind].length)
            )
...
def small_rook_to_queen(geometries0_df, min_rook_length):
    ...
    adj_df = adjacencies(
        geometries_df, output_type="geodataframe"
    )  # We're assuming the input is clean, so these should all be 1-D or less
    ...
    for (
        a_ind
    ) in small_adj_df.index:  # Don't put a progress bar here; it gets out of control!
        ...
        adj_len = (
            geometries_df["geometry"][geoms_for_this_adj[0]]
            .intersection(geometries_df["geometry"][geoms_for_this_adj[1]])
            .length
        )
        ...
        if adj_len > 0 and adj_len < min_rook_length:
            ...
            possible_geom_integer_indices = [
                *set(
                    numpy.ndarray.flatten(
                        g_spatial_index.query(
                            small_adj_df["geometry"][a_ind].buffer(2 * fat_point_radius)
                        )
                    )
                )
            ]
            ...
            pieces_df = geopandas.GeoDataFrame(
                ...
                geometry=geopandas.GeoSeries(
                    [geom for geom in polygonize(boundaries_union)]
                ),
                crs=geometries_df.crs,
            )
            ...
            for i in pieces_df.index:
                ...
                for j in temp_possible_geom_indices:
                    if (
                        ...
                        pieces_df["polygon indices"][i] = pieces_df["polygon indices"][
                            i
                        ].union({j})
            ...
            for p_ind in pieces_df.index:
                if (
                    ...
                    disk_to_remove = unary_union(
                        [disk_to_remove, pieces_df["geometry"][p_ind]]
                    )
            ...
            circle_boundaries_df = intersections(
                geopandas.GeoDataFrame(
                    geometry=geopandas.GeoSeries(
                        [disk_to_remove], crs=geometries_df.crs
                    )
                ),
                possible_geoms,
            ...
            for b_ind in circle_boundaries_df.index:
                if (
                    circle_boundaries_df["geometry"][b_ind].geom_type
                    == "MultiLineString"
                ):
                    circle_boundaries_df["geometry"][b_ind] = linemerge(
                        circle_boundaries_df["geometry"][b_ind]
                    )

            circle_boundaries_df = circle_boundaries_df.explode(
                index_parts=False
