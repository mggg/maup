...
def toy_counties_geodataframe():
    ...
    return geopandas.GeoDataFrame(geometry = geopandas.GeoSeries([cpoly1, cpoly2, cpoly3, cpoly4]))
...
class TestSmartRepair:
    ...
    def test_nest_within_regions(self, toy_precincts_geodataframe, toy_counties_geodataframe):
        repaired_with_regions_gdf = smart_repair(toy_precincts_geodataframe, 
                                                nest_within_regions = toy_counties_geodataframe
                                                )
        p_to_c = assign(toy_precincts_geodataframe, toy_counties_geodataframe)
        for p in p_to_c.index:
            assert toy_counties_geodataframe.geometry[p_to_c[p]].contains(repaired_with_regions_gdf.geometry[p])
    ...
    def test_small_rook_to_queen(self, toy_precincts_geodataframe):
        ...
        repaired_srtq_gdf = smart_repair(toy_precincts_geodataframe, min_rook_length=0.05)
